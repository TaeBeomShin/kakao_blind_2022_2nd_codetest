...
def changegrade_api(auth_key,grade_data):
    ...
    userinfo_response=requests.put(base_url+"/change_grade",headers=headers,data=put_parameter)
    userinfo_data=userinfo_response.json()
    userinfo_json=json.loads(json.dumps(userinfo_data))
...
for i in range(0,596):
    if(i==0):
        changegrade_api(problem2_auth_key,make_to_json_object(user_dict))
    
    current_match=[]
    match_needed_group={};
    waiting_dict={};

    waitingline_info=waitingline_api(problem2_auth_key)
    ...
    게임결과를 받아서 점수를 계산한다.
    '''
    gameresult_info=gameresult_api(problem2_auth_key).get("game_result")
    ...
    for id,start_time in waiting_dict.items():
        ...
        2. 매칭 대기시간이 5분지나면 매칭 필요한 그룹에 넣음.
        '''
        if(battle_dict[id]<=3 or i-start_time>5):
    ...
    res=match_api(problem2_auth_key,current_match)
    print(res)
    changegrade_api(problem2_auth_key,make_to_json_object(user_dict))
    

userinfo=userinfo_api(problem2_auth_key)
score=score_api(problem2_auth_key)
print(score)
# problem2_auth_key=get_start_key("2")